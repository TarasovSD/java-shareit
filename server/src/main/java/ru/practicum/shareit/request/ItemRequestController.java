...
public class ItemRequestController {
    ...
    @GetMapping()
    public List<ItemRequestDtoWithResponses> getListOfOwnRequests(@RequestHeader("X-Sharer-User-Id") Long userId) {
        log.info("Выполнен запрос списка всех запосов пользователя с ID: " + userId);
        return requestService.getListOfOwnRequests(userId);
    }

    @GetMapping("/all")
    public List<ItemRequestDtoWithResponses> getListOfAllRequests(@RequestHeader("X-Sharer-User-Id") Long userId,
